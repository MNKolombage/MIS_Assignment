...
import { ChevronDown, Plus, Edit2, Trash2, AlertCircle, Star } from "lucide-react"
...
export default function RoomsManagement() {
  const [rooms, setRooms] = useState<Room[]>([
    ...
    },
    {
      ...
      beds: 2,
      bathrooms: 2,
      amenities: ["WiFi", "Pool Access", "Beach Access", "AC", "Outdoor Shower"],
      rating: 4.9,
      reviews: 56,
  ...
  const statuses = ["All", "Available", "Occupied", "Maintenance", "Cleaning", "Blocked"]

  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold text-gray-900">Rooms Management</h2>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase">Price/Night</th>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase">Capacity</th>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase">Status</th>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500 uppercase">Rating</th>
            ...
            {filteredRooms.map((room) => (
              <div key={room.id}>
                <tr className="hover:bg-gray-50 transition cursor-pointer">
                  ...
                  <td className="px-6 py-4 font-semibold text-gray-900">${room.price}</td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      <ChevronDown size={18} className={`transition ${expandedRoom === room.id ? "rotate-180" : ""}`} />
                    </button>
                  </td>
                </tr>
                    ...
                    <td colSpan={7} className="px-6 py-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Last: {room.lastMaintenance}</p>
                          {room.nextMaintenance && (
                            <p className="text-sm text-gray-600">Next: {room.nextMaintenance}</p>
                          )}
                        </div>
                        {room.currentGuest && (
                          <div>
                            <h4 className="font-semibold text-gray-900 mb-3">Current Guest</h4>
